...
include_once( __DIR__."ufpdf2.php");
...
global $woocommerce;
        $total = 0;
            $statuses = array('pending' => 'В ожидании оплаты',
...
$pageTitle = get_bloginfo('name')." Данные Заказа".$order_id;
 ...
 //получить товары заказа
  $order_items = $order->get_items();
   $data = $order->get_data();
    ...
    $pdf->SetFontSize(18);
    $pdf->SetXY(10,10);
	$pdf->SetTitle(iconv("utf-8", "utf-8", "Order details " . $order_id), true);
    $pdf->Ln( $reportNameYPos );
$pdf->Cell( 0, 0, iconv("utf-8", "windows-1251", "Данные вашего заказа"), 0, 0, 'C' );
//калантитул
//товары
...
//$pdf->Ln( 16 );
//$pdf->SetFont( 'georgia', '', 12 );
//$pdf->Write( 6, iconv("utf-8", "windows-1251", get_post_meta($order_id, 'billing_payment', true)));
$pdf->Ln( 16 );
$pdf->Write( 6, iconv("utf-8", "windows-1251", "Сроки оплаты: ".$data['date_modified']->date('d.m.Y')));
$pdf->Ln( 16 );
$pdf->Write( 6, iconv("utf-8", "windows-1251", "Статус заказа: ".$statuses[$data['status']]));
$pdf->Ln( 16 );
